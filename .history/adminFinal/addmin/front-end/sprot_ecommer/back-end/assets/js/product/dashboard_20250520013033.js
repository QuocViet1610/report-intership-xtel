...
$.ajax({
    ...
    success: function (response) {
        if (response.code === "200" && response.data) {
            ...
            const data = response.data; // Mảng ProductViewDto
                         console.log("data");
                console.log(data);
            data.forEach(product => {
                // productImages là Set hoặc Array, chuyển về Array nếu cần
                const imagesArray = Array.isArray(product.productImages) 
                    ? product.productImages 
                    : Array.from(product.productImages || []);

                // Tìm ảnh chính (isPrimary = 1), hoặc ảnh đầu tiên nếu không có
                const primaryImageObj = imagesArray.find(img => img.isPrimary === 1) || imagesArray[0];
                const primaryImageUrl = primaryImageObj ? primaryImageObj.imageUrl : 'default-image.png'; // fallback ảnh
                ...
                $('#product-table-body').append(`
                    <tr>
                        ...
                        <td>
                            <div class="product-detail-box">
                                <h6>Giá vốn</h6>
                                <h5>${product.productCostPrice?.toLocaleString() || 0}đ</h5>
                            </div>
                        </td>
                        <td>
                            <div class="product-detail-box">
            ...
            // Xử lý khi không có data hoặc lỗi code
