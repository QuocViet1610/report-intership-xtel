                    ...
                    <tr class="product-variants child-row product-last" data-parent="${product.id}" style="display: none; border-right: 1px solid green;">
                        ...
                        <td colspan="12" style="text-align: center; font-weight: bold; border-right: 1px solid green; border-bottom: none;">
                            <div style="display: flex; justify-content: right;">
                                ${product.isActive == 0 ? '<button class="btn btn-update btn-solid">Đã ngừng kinh doanh</button>' : ''}
                            </div>
                        </td>
                    </tr>
...
function filterCategoriesByKeyword(keyword) {
    ...
    let matched = allCategories.filter(cat => 
    ...
    // Hàm đệ quy để thêm category và tất cả cha của nó
    function addWithParents(cat) {
        if (!cat) return;
        keepIds.add(cat.id);
        if (cat.parentId && cat.parentId !== 0) {
            const parent = categoryMap[cat.parentId];
            if (parent && !keepIds.has(parent.id)) {
                addWithParents(parent);
            }
        }
    }

    // Thêm tất cả category khớp và cha của chúng
    matched.forEach(cat => addWithParents(cat));

    // Lọc danh sách đầy đủ để chỉ giữ các category khớp và cha của chúng
