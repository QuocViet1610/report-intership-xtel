...
$.ajax({
   ...
   success: function (response) {
    ...
    if (!data || data.length === 0) {
        $('#product-table-body-user').append('<tr><td colspan="7" class="text-center">Không có dữ liệu</td></tr>');
        return;
    }

    data.forEach(row => {
        // row = [userId, fullName, avatar, email, phone, totalProductsBought, totalAmountSpent]
        const userId = row[0];
        const fullName = row[1];
        var avatar = row[2] || 'product/avatar-vo-danh-9.png'
        avatar = "http://localhost:9000/sports/" + avatar || 'http://localhost:9000/sports/product/avatar-vo-danh-9.png'; // Ảnh đại diện fallback
        const email = row[3] || '-';
        const phone = row[4] || '-';
        const totalProductsBought = row[5];
        const totalAmountSpent = row[6];

        const orderDate = '-';    // nếu không có dữ liệu thì để tạm
        const orderStatus = '-';

        $('#product-table-body-user').append(`
            <tr>
                <td>
                    <div class="best-product-box" style="display: flex; align-items: center; gap: 10px;">
