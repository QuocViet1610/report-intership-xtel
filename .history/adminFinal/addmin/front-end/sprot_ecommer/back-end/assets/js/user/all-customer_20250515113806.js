...
function fetchUserData() {
    ...
    $.ajax({
        url: 'http://localhost:8080/user/get-usr',
        type: 'GET',
...
function renderUserTable(page, size) {
    ...
    currentPageData.forEach(user => {
        const totalSpentFormatted = new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(user.totalSpent);
        ...
        $('#user-table-body').append(`
            <tr>
                ...
                <td class="td-check">${statusIcon}</td>
                   <td class="td-check">${user.totalOrders}</td>

                          <td class="td-check">${totalSpentFormatted}</td>
                <td>
                    <ul>
...
$('#confirmToggleLock').on('click', function() {
    if (!selectedUserId) return;

    let token = localStorage.getItem("authToken");
    $.ajax({
        url: `http://localhost:8080/user/update-active/${selectedUserId}`,
        type: 'PUT',
        contentType: 'application/json',
        headers: {
            "Authorization": "Bearer " + token,
        ...
        success: function(response) {
            if (response.code === "200") {
                const userIndexAll = allUserData.findIndex(user => user.id === selectedUserId);
                const userIndexFiltered = filteredUserData.findIndex(user => user.id === selectedUserId);
...
fetchUserData();
    ...
    // }, 1000);
            ...
            const modalEl = document.getElementById('confirmLockModal');
...
function setupFilterEvents() {
    ...
    $("#order-date").on("change", function() {
        ...
        // Create a copy of the current filtered data
        let sortedData = [...filteredUserData];
        
        // Sort based on selected option
        switch(sortOption) {
            case "newest": // Tổng đơn hàng từ cao đến thấp
                sortedData.sort((a, b) => (b.totalOrders || 0) - (a.totalOrders || 0));
                break;
            case "oldest": // Tổng đơn hàng từ thấp đến cao
                sortedData.sort((a, b) => (a.totalOrders || 0) - (b.totalOrders || 0));
                break;
            case "ascending": // Tổng giá đơn hàng từ cao đến thấp
                sortedData.sort((a, b) => (b.totalSpent || 0) - (a.totalSpent || 0));
                break;
            case "descending": // Tổng giá đơn hàng từ thấp đến cao
                sortedData.sort((a, b) => (a.totalSpent || 0) - (b.totalSpent || 0));
                break;
            default:
...
// Modify your setupSearchFunction to maintain the sort option during search
function setupSearchFunction() {
    // Handle search input with debounce effect
    $("#searchInput").on("input", function() {
        clearTimeout(searchTimer);
        ...
        searchTimer = setTimeout(function() {
            const searchText = $("#searchInput").val().toLowerCase().trim();
            const currentSortOption = $("#order-date").val(); // Get current sort option
            ...
            if (currentSortOption) {
                // Trigger the sort function again to maintain sort order
                $("#order-date").trigger("change");
                return; // The change event will handle rendering and pagination
            }
...
// Update your document ready function to call the new setup function
