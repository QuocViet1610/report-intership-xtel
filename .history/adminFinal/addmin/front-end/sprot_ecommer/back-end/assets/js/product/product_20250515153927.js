...
function handleCategoriesResponse() {
    $.ajax({
        ...
        success: function(response) {
            if(response.code == 200) {
                allCategories = response.data;  // Lưu toàn bộ dữ liệu
...
function filterCategoriesByKeyword(keyword) {
    ...
    // Lọc các thể loại mà tên hoặc tên cha có chứa từ khóa
    // Cách đơn giản: lọc category name chứa keyword
    const filtered = allCategories.filter(cat => cat.name.toLowerCase().includes(lowerKeyword));
    
    // Nếu muốn hiển thị luôn cha con (phân cấp) của filtered thì cần xử lý phức tạp hơn,
    // ở đây ta chỉ show category lọc được (không show cha con)
    renderCategoryFilter(filtered);
}
