...
$.ajax({
   ...
   success: function (response) {
    ...
    data.forEach(row => {
        ...
        const avatar = "http://localhost:9000/sports/" + row[2] || 'default-avatar.png'; // Ảnh đại diện fallback
        const email = row[3] || '-';
        const phone = row[4] || '-';
...
$.ajax({
    url: `http://localhost:8080/product/search?page=0&size=5&sort=id.asc`,
            method: "POST",
            dataType: "json",
