...
let userData = []; // Store all user data
...
function renderUserTable(page, size) {
    ...
    const endIndex = Math.min(startIndex + size, userData.length);
...
function setupActionEvents() {
    ...
    $('#user-table-body').on('click', '#toggle-lock', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.code === "200") {
                    // Update user data in our local array instead of reloading the page
                    const userIndex = userData.findIndex(user => user.id === userId);
                    if (userIndex >= 0) {
                        // Toggle isActive status
                        userData[userIndex].isActive = userData[userIndex].isActive === 1 ? 0 : 1;
    ...
    $('#delete-user').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                // Remove deleted user from our data array
