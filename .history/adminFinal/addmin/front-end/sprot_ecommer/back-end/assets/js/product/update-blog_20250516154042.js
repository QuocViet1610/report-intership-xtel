function fetchUserData() {
  ...
  // Wait a short time to ensure all editors are loaded
  setTimeout(() => {
    // Use a simpler selector that's more likely to match
    const editors = document.querySelectorAll('.ck-editor');
    console.log('Found editors:', editors.length);
    
    if (editors.length >= 2) {
      // Log before removing to debug
      console.log('Removing editor:', editors[1]);
      editors[1].remove();
    }
