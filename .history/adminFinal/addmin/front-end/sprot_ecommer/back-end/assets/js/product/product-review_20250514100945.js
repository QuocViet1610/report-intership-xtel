    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            reviews.forEach(review => {
                console.log("fffff")
                console.log(review.isActive)

let statusIcon = review.isActive === 1
    ? '<i class="ri-checkbox-circle-line" style="#0da487 !important;" ></i>'
...
let token = localStorage.getItem("authToken");
    ...
    $.ajax({
        ...
        success: function(response) {
         ...
         if (newStatus == 1) {
    $(`[data-id=${reviewId}]`).closest('tr').find('.td-check i')
        .removeClass('ri-close-circle-line')
        .addClass('ri-checkbox-circle-line')
        .css('color', '#0da487'); // Apply green color
} else {
    $(`[data-id=${reviewId}]`).closest('tr').find('.td-check i')
        .removeClass('ri-checkbox-circle-line')
        .addClass('ri-close-circle-line')
        .css('color', 'red'); // Apply red color
}
