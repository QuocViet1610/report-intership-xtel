...
fetchDataApi(null, null, null, 'productId.desc');

function fetchDataApi(brandId, attributeArray, genderId, sort) {
 ...
 var idCate= id;
   var sortParam = sort || 'productId.desc'; 
    ...
    $.ajax({
        url: "http://localhost:8080/product/search?page=0&size=20&sort=" + sortParam,
        type: "POST", 
        contentType: "application/json",  
        ...
        success: function(response) {
            if (response.data && response.data.content) {
                ...
                renderProducts(response.data.content, idCate); // Hiển thị kết quả tìm kiếm
            }
        },
        error: function(xhr, status, error) {
            let errorMessage = "Lịch sử giao hàng đang trống";
...
$('.brandlisst').on('change', 'input[type="radio"]', function() {
    ...
    fetchDataApi(brand ,selectedItems,object,sort,searchProduct);  
});
...
$('.js-range-slider').on('change', function() {
   ...
   fetchDataApi(brand,selectedItems,object,sort,searchProduct);
    ...
    document.getElementById('zToa').addEventListener('click', function() {
        console.log('Sắp xếp Z - A đã được chọn');
