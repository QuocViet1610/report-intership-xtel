...
var password= 1234568;
 ...
 if (data.avatar) {
            $("img.update_img").attr("src", data.avatar);  // Set avatar image
        }
...
function readURL(input) {
    ...
    if (file) {
        ...
        // Optionally, you can send the image to the server using AJAX after selecting
        const formData = new FormData();
        formData.append('image', file);  // 'image' should match the parameter name in your backend method
        ...
        if (token) {
            ...
            const base64 = base64Url.replace(/-/g, '+').replace(/_/g, '/'); // Fix for base64 URL encoding
            const decodedPayload = JSON.parse(atob(base64)); // Decode base64 and parse as JSON

            // Access the userId or other fields in the decoded token
        ...
        $.ajax({
            url: `http://localhost:8080/user/${userId}`,  // Replace `id` with the actual ID you want to update
            type: 'PUT',
            contentType: false,  // Don't set contentType because FormData will set it automatically
