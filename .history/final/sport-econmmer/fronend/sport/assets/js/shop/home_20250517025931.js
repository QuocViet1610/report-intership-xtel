...
$(document).on('click', '.notifi-wishlist', function(e) {
    ...
    const token = $this.data('token') || $this.attr('data-token');
     const price = $this.data('price') || $this.attr('data-price');
    ...
    saveFavoriteProduct(name, image, article, token, price);
});

// Tách logic lưu sản phẩm yêu thích thành hàm riêng
function saveFavoriteProduct(name, image, article, token, price) {
