...
$(document).on('click', '.notifi-wishlist', function() {
    ...
    // Ví dụ lưu vào localStorage (hoặc gửi lên API)
    const favoriteProduct = { name, image, article, token };
    ...
    // Hoặc gửi lên server qua ajax
    /*
    $.ajax({
        url: '/api/save-favorite',
        method: 'POST',
        ...
        error: function(err) {
            alert('Lỗi khi lưu yêu thích!');
        }
    });
    */

    alert(`Đã lưu sản phẩm yêu thích:\n${name}`);
});
