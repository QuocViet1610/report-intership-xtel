        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.code === "200" && response.data) {
                    ...
                    localStorage.setItem('discountValue', coupon.discountValue);
                    localStorage.setItem('couponId', coupon.couponId);

                    const discountText = `(-) ${coupon.discountValue.toLocaleString()}đ`;  // Định dạng lại giá trị nếu cần
                    document.getElementById('coupon').innerText = discountText;
                    ...
                    const finalPrice = provisionalPrice - coupon.discountValue;
                    const finalPriceText = finalPrice.toLocaleString() + 'đ';

                    // Cập nhật giá trị vào phần tử '#total'
                    document.getElementById('total').innerText = finalPriceText;  
