...
var password= 1234568;
            ...
            $(".btn.theme-bg-color").click(function() {
                ...
                // Gửi yêu cầu PUT đến API change-password-update
                $.ajax({
                    url: 'http://localhost:8080/user/change-password-update', // URL API
                    type: 'PUT',
                    headers: {
                        "Authorization": "Bearer " + token,  // Nếu cần token
                        "Accept": "application/json",
                        'passwordOld': passwordOld,  // Truyền passwordOld lên header
                        'passwordNew': passwordNew   // Truyền passwordNew lên header
                    },
                    success: function(response) {
                        // Hiển thị thông báo thành công
                        showSuccess("Mật khẩu đã được thay đổi thành công!");
                        // Đóng modal sau khi thành công
                        $('#editPasword').modal('hide');
                    },
                    error: function(xhr, status, error) {
