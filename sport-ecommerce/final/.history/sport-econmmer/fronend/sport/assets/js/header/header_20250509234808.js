...
$(document).ready(function () {
    ...
    $('.logout').on('click', function() {
        ...
        localStorage.removeItem('username');  // If you stored the username as well
    
        // Redirect to index-5.html
        window.location.href = 'index-5.html';  // Redirect to the desired page
    });

// search product
    document.getElementById('button-addon2').addEventListener('click', function() {
        var searchQuery = document.getElementById('searchInput').value; // Lấy giá trị từ ô tìm kiếm
        if (searchQuery) {
            // Nếu có từ khóa tìm kiếm, chuyển hướng đến trang chi tiết sản phẩm
            window.location.href = "shop-left-sidebar.html?search=" + encodeURIComponent(searchQuery);
        } else {
...
function renderCartHeader() {
    ...
    if (!token) {
        $('#cart-button-header').attr('href', 'login.html'); 
        $('#checkout-button-header').attr('href', 'login.html'); 
    }
