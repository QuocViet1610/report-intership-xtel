        ...
        function addressDetail(id) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Gán tên người mua và số điện thoại vào thẻ HTML
                    document.querySelector('.order-details-name h4:nth-child(2)').textContent = fullName;
                    document.getElementById('phoneNumber').textContent = phoneNumber; // Hiển thị số điện thoại
        
                    // Gán địa chỉ đầy đủ vào thẻ có id là estimatedTime
                    document.querySelector('.order-details-name h4:nth-child(3)').textContent = fullAddress;
