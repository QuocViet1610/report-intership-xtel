...
$(document).ready(function (){
    ...
    ocument.getElementById('applyCouponBtn').addEventListener('click', function() {
        ...
        $.ajax({
            url: `http://localhost:8080/detail?code=${couponCode}`, // API với tham số code
            type: "GET",
            headers: {
            ...
            success: function(response) {
                if (response.code === "200" && response.data) {
                    ...
                    alert('Mã giảm giá hợp lệ! Tên mã giảm giá: ' + coupon.name);
                    
                    // Bạn có thể xử lý thêm dữ liệu từ `coupon` ở đây, ví dụ:
                    // - Hiển thị thông tin mã giảm giá trong giao diện
                    // - Cập nhật giá trị giỏ hàng
                } else {
