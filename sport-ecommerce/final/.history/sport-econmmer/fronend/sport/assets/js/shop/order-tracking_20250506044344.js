        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                for (let i = 0; i < orderDetail.length; i++) {
                    ...
                    let product = {
                        productImage: item.productView.find(image => image.isPrimary === 1),
                        productName: item.productView.productName,
                        color: item.productView.productVariants[0].variantAttributes.find(attr => attr.attributeName === 'Màu sắc').attributeValues[0].name,
                        size: item.productView.productVariants[0].variantAttributes.find(attr => attr.attributeName === 'Kích thước').attributeValues[0].name,
                        price: item.price,
