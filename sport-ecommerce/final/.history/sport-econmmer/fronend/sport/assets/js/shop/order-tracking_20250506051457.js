    ...
    if(token){
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let orderDetail = response.data.orderDetails;
                ...
                // Hàm để tạo các dòng cho mỗi sản phẩm
                function createProductRow(item) {
                    let productRowHTML = '';
                    
                    // Lấy số lượng sản phẩm và thông tin khác
                    let quantityProduct = item.quantity;
                    let productView = item.productView;
                    let mainImage = productView.productImages.find(image => image.isPrimary === 1);
                    let imageUrl = mainImage ? mainImage.imageUrl : 'default-image.jpg'; // Nếu không có ảnh chính, dùng ảnh mặc định
                    let productName = productView.productName;
                    let productPrice = productView.productPrice;
                
                    // Tính tổng giá sản phẩm
                    let totalPrice = parseFloat(productPrice) * parseInt(quantityProduct);
                    ...
                    productRowHTML += `
                        <tr>
                            <td class="product-detail">
                                <div class="product border-0">
                                    <a href="product.left-sidebar.html" class="product-image">
                                        <img src="${imageUrl}" class="img-fluid blur-up lazyloaded" alt="">
                                    </a>
                                    <div class="product-detail">
                                        <ul>
                                            ...
                                            <li class="text-content">Màu sắc: ${productView.productVariants[0].variantAttributes[0].attributeValues[0].name}</li>
                                            <li class="text-content">Kích cỡ: ${productView.productVariants[0].variantAttributes[1].attributeValues[0].name}</li>
                                        </ul>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="price">
                                ...
                                <h6 class="theme-color">${productPrice.toLocaleString()}đ</h6>
                            </td>
                    ...
                    // Gán dòng HTML vào bảng
                    document.querySelector("tbody").innerHTML += productRowHTML;
                
                    // Cộng tổng số tiền cho tất cả các sản phẩm
                    totalAmount += totalPrice;
                }
                
                // Lặp qua từng sản phẩm trong đơn hàng và tạo các dòng sản phẩm
