...
$(document).ready(function (){
    ...
    document.getElementById('applyCouponBtn').addEventListener('click', function() {
        ...
        // Gọi API để lấy chi tiết mã giảm giá
        $.ajax({
            url: `http://localhost:8080/coupon/detail?code=${couponCode}`, // API với tham số code
            type: "GET",
            headers: {
                "Authorization": "Bearer " + token, // Nếu cần token, thêm vào header
   ...
   // Lấy giá trị từ phần tử 'provisionalPrice' (ví dụ: 1.798.000đ)
   const provisionalPriceText = document.getElementById('provisionalPrice').innerText;
   // Chuyển đổi giá trị từ chuỗi "1.798.000đ" thành số để dễ dàng tính toán
   const provisionalPrice = parseFloat(provisionalPriceText.replace(/\./g, '').replace('đ', ''));

   // Trừ giá trị giảm giá từ giá ban đầu
   const finalPrice = provisionalPrice - discountValue;

   // Định dạng giá trị kết quả cuối cùng
   const finalPriceText = finalPrice.toLocaleString() + 'đ';

   // Cập nhật giá trị vào phần tử '#total'
   document.getElementById('total').innerText = finalPriceText;  // Cập nhật giá mới vào phần tử 'total'
