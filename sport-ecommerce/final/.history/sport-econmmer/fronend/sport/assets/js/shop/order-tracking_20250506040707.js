        ...
        function addressDetail(id) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Gộp các thông tin lại thành một chuỗi duy nhất
                    const fullAddress = `${addressText}, ${wardName}, ${districtName}, ${provinceName}`;
                    document.querySelector('.order-details-name h4:nth-child(2)').textContent = response.data.fullName; // Gán tên người mua
            document.querySelector('.order-details-name h4:nth-child(3)').textContent = fullAddress; // Gán địa chỉ đầy đủ

            console.log("Địa chỉ đầy đủ: ", fullAddress);  // In địa chỉ đầy đủ ra console
