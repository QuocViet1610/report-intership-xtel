        ...
        $('#pills-order-tab').click();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#dob').text(formatDate(data.createdAt));  // Gán ngày sinh (giả sử ngày sinh là ngày tạo)
                    $('#phone').html(`<a href="javascript:void(0)">${data.phone}</a>`);  // Gán số điện thoại
                    $('#email').text(data.email);  // Gán email
            
                    // Thông tin đăng nhập
                    $('#loginEmail').html(`<a href="javascript:void(0)">${data.email}</a>`);  // Gán email đăng nhập
