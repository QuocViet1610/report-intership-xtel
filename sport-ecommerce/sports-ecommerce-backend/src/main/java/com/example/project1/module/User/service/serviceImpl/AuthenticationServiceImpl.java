...
import com.example.project1.module.User.repository.*;
...
//        userVerificationService.saveOtp(request, otp);
    ...
    @Override
    public void logout(IntrospectionRequest request) {
        try {
            boolean signTokenValidate = tokenUtil.validateToken(request.getToken());

            if (!signTokenValidate){
                throw new ValidateException(Translator.toMessage("Tài khoản không tồn tại"));
            }
        ...
        } catch (AppenderLoggingException exception) {
    ...
    @Override
    public String refreshToken(IntrospectionRequest request) {
        String token = request.getToken();
        // logout token cu
        // tao ra token moi (refresh token co thoi gian su dung dai hon token)
        return null;
    }
