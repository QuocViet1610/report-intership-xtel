...
$('#sizeClothes, #sizeShoes, #colorsearch').on('change', 'input[type="checkbox"]', function() {
    ...
    if ($(this).is(':checked')) {
        // Nếu checkbox được tích, thêm giá trị vào mảng
        selectedItems.push(checkboxValue);
    } else {
        // Nếu checkbox không được tích, loại bỏ giá trị khỏi mảng
        selectedItems = selectedItems.filter(item => item !== checkboxValue);
    }
