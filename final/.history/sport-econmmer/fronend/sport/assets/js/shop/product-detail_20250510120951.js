        ...
        if (response.data && response.data.content) {
           ...
           if (Number(productId) !== Number(product.productId)){
            ...
            $('.product-list').html(productListHTML);
              ...
              var productListHTML2 = '<section class="product-list-section section-b-space"><div class="container-fluid-lg"><div class="title"><h2>Sản phẩm cùng thể loại</h2><span class="title-leaf"><svg class="icon-width"><use xlink:href="https://themes.pixelstrap.com/fastkart/assets/svg/leaf.svg#leaf"></use></svg></span></div><div class="row"><div class="col-12"><div class="slider-6_1 product-wrapper">';

            // Loop through products and build HTML for each
            products.forEach(function(product) {
                var imageUrl = 'default-image.jpg'; // Default image if not found
                if (product.productImages && product.productImages.length > 0) {
                ...
                var productName = product.productName || 'Tên sản phẩm';
                var productPrice = product.productPrice || 0;  // Default to 0 if no price
                var productLink = 'product-left-thumbnail.html?id=' + product.productId + '&cateId=' + cateId;

                // Add each product's HTML to the list
                productListHTML2 += `
                    <div>
                        <div class="product-box-3 wow fadeInUp">
                            ...
                            <div class="product-footer">
                                <div class="product-detail">
                                    ...
                                    <h6 class="unit"></h6>
                                    <h5 class="price"><span class="theme-color">${productPrice.toLocaleString()}đ</span></h5>
            ...
            productListHTM2L += '</div></div></div></div></section>';

            // Inject the product list HTML into the DOM
            $('.product-list').html(productListHTML2);
        }
    },
    error: function(xhr, status, error) {
