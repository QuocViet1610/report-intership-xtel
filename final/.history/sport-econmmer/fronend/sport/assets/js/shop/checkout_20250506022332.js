...
function displayCheckout(cart) {
    ...
    const discountValue = parseFloat(discountValueFromStorage);
...
$(document).ready(function() {
    ...
    $('#paymentButton').on('click', function() {
        ...
        if ($('#cash').is(':checked')) {
            ...
            const discountText = document.querySelector('.summery-total li:nth-child(3) .price').innerText;
            const discountValue = parseFloat(discountText.replace(/\./g, '').replace('đ', '').trim());
