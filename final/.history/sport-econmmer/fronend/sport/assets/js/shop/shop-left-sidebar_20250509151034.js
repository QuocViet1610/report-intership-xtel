                ...
                brandListHTML += `
                    <li>
                        <div class="form-check ps-0 m-0 category-list-box">
                            <input class="checkbox_animated" type="checkbox" id="flexCheckDefault${brand.id}" data-brand-id="${brand.id}">
...
// Mã lọc theo brandId
function fetchDataApi(brandId) {
    var requestData = {
        searchText: "",
        fullParentId: id,
        brandId:    parseInt( brandId) || null,  // Lọc theo brandId
    };
...
// Lắng nghe sự kiện khi người dùng thay đổi lựa chọn brand
$('.brandlisst').on('change', 'input[type="checkbox"]', function() {
    console.log("a")
    let selectedBrandId = [];

    // Lọc các checkbox đã được chọn và lấy id brand
    $('input[type="checkbox"]:checked').each(function() {
        selectedBrandId.push($(this).data('brand-id'));
    });

    // Gọi hàm fetchDataApi với brandId đã chọn
    fetchDataApi(selectedBrandId.length > 0 ? selectedBrandId : null);  // Gửi mảng các brandId, nếu không có gì thì gửi null
});
